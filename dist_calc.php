<!-- //connect and get model names frm Database -->
<?php
$server = "localhost";
$username = "root";
$password = "";
  ...
  <div class="header">
    <div class="container">
      ...
      <div class="forms" style="display:flex;justify-content:space-evenly;">
        <form style="font-size:22px" id="vehicle-form" action="dist_calc.php" method="post">
          <h1 style="margin:20px 0 15px 10px;font-size:35px;">1. Select your vehicle: </h1><br>
          <?php foreach ($result as $vehicle) { ?>
            <label>
              <input type="radio" name="selectedVehicle" value="<?php echo $vehicle['model_name'] ?>">
              <?php echo $vehicle['model_name'] ?>
            </label><br>
          <?php } ?><input type="radio" name="selectedVehicle" value='other'><b>other</b>
          <button class='btn' type="submit" style='margin: 40px 0 0 55px;' name='submit'>Submit</button>
        </form>
        <form id="trip-form" style="margin: 20px 0 0 20px;">
          <h1 style="margin:5px 0 0 25px; font-size:35px;">2. Driving Distance Calculator</h1>
          <div class="form">
      ...
      if (isset($_POST['submit'])) {
        if (isset($_POST['selectedVehicle'])) {
          $selected_model_name = $_POST['selectedVehicle'];
          if ($selected_model_name != 'other') {
            $sql = "SELECT * FROM `ev`.`electric_bikes` WHERE model_name = '$selected_model_name'";
            $result = $db->query($sql);
            if ($result) {
              $bike = $result->fetch_assoc();
              $real_range = $bike['real_range'];
              $battery_cap = $bike['battery_cap'];
              $charging_time = $bike['charging_time'];
              echo "<p style='font-size:20px;margin:60px 0 0 100px;'>Vehicle Selected : " . $selected_model_name . "</p>";
              echo "<script>";
              echo "selectedVehicle = '$selected_model_name';";
            ...
            // Display form for "other" option
            echo "<form method='post' action='other.php' style='margin:55px;'>";
            echo '<h3 style="margin-bottom:20px;"><b style="color:black;">Enter vehicle details</b></h3>';
            echo '<input type="text" name="battery_cap" id="battery_cap" placeholder="Battery capacity in kW"style="margin:0 10px;">';
            echo '<input type="text" name="real_range" id="real_range" placeholder="Average range in kms"style="margin:0 10px;">';
            echo '<input type="text" name="charging_time" id="charging_time" placeholder="Charging time in hh:mm:ss"style="margin:0 10px;">';
            echo '<div class="btn-group"><button class="btn submit-btn" type="submit"style="margin:0 10px;">Submit</button><button class="btn reset-btn" type="reset">Reset</button></div>';
            echo '</form>';
          }}
          else echo "<p style='color:red;margin:50px 0 0 125px;'>Please select a vehicle</p>";
        }
      $db->close();
      ?>
